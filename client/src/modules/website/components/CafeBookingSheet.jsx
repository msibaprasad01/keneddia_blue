...
const isCafeType = (value = "") => String(value).toLowerCase().trim() === "cafe";
...
export default function CafeBookingSheet({ isOpen, onOpenChange }) {
  ...
  const fetchProperties = async (type) => {
    ...
    try {
      ...
      const filtered = (res.data ?? []).filter(
        (p) =>
          p.isActive &&
          p.propertyTypes?.some((t) => isCafeType(t)),
      );

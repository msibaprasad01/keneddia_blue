...
import { Plus, Edit, MapPin, Sparkles, ChevronLeft, ChevronRight, Loader2, Calendar, Clock, Building2, Ticket, Search, ToggleLeft, ToggleRight } from 'lucide-react';
import { getDailyOffers, updateDailyOfferActiveStatus } from '@/Api/Api';
...
function DailyOffers() {
  ...
  const fetchOffers = async () => {
    try {
      ...
      setAllOffers([...offersData].reverse());
    } catch (error) {
    ...
    <div className="p-1">
      ...
      <div className="rounded-xl shadow-sm border overflow-hidden" style={{ backgroundColor: colors.contentBg, borderColor: colors.border }}>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b" style={{ backgroundColor: colors.mainBg, borderColor: colors.border }}>
                  ...
                  <th className="text-left py-4 px-4 text-[11px] font-bold uppercase tracking-wider text-gray-500">Property Type</th>
              ...
              <tbody>
                {paginatedOffers.map((offer) => (
                  <tr key={offer.id} className="border-b hover:bg-gray-50/50 transition-colors" style={{ borderColor: colors.border }}>
                    ...
                    <td className="py-3 px-4">
                      <div className="max-w-[200px]">
                        <h4 className="font-semibold text-sm truncate" style={{ color: colors.textPrimary }}>{offer.title}</h4>
                        <p className="text-[11px] text-gray-500 line-clamp-1">{offer.description}</p>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4 text-center">
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold transition-all cursor-pointer disabled:cursor-wait ${
                          offer.isActive ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-600 hover:bg-red-200'
                        }`}
                      >
        ...
        {!loading && totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t bg-gray-50" style={{ borderColor: colors.border }}>
